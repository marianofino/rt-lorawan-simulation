...
public:
  ...
  void SetPeriod (std::vector<double>& periods);

  void SetPacketSizeRandomVariable (Ptr <RandomVariableStream> rv);

  void SetInitialDelay (std::vector<double>& initialDelays);
  void SetSFs (std::vector<int>& sf);

  void SetPacketSize (uint8_t size);


private:
  Ptr<Application> InstallPriv (Ptr<Node> node, int i) const;

  ObjectFactory m_factory;

  std::vector<double> m_initialDelays;
  //Time m_initialDelay;
