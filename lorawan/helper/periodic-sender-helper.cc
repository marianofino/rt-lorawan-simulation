...
{
  return ApplicationContainer (InstallPriv (node, 0));
}
...
Ptr<Application>
PeriodicSenderHelper::InstallPriv (Ptr<Node> node, int i) const
{
  NS_LOG_FUNCTION (this << node);

  Ptr<PeriodicSender> app = m_factory.Create<PeriodicSender> ();

  Time interval = Seconds(m_periods[i]);
  /*
  ...
  */
  ...
  app->SetInitialDelay (Seconds(m_initialDelays[i])); // m_initialDelay->GetValue (0, interval.GetSeconds ())
  
  // set max payload packet size
  std::vector<uint32_t> packetSizesBySF = { 230, 230, 123, 59, 59, 59 }; // SF7, SF8, SF9, SF10, SF11 and SF12
  uint8_t packetSize = packetSizesBySF[m_sf[i] - 7];
...
void
PeriodicSenderHelper::SetPeriod (std::vector<double>& periods)
{
  //m_period = period;
  m_periods = periods;
}
...
void
PeriodicSenderHelper::SetInitialDelay (std::vector<double>& initialDelays)
{
  m_initialDelays = initialDelays;
}

void
PeriodicSenderHelper::SetSFs (std::vector<int>& sf)
{
  m_sf = sf;
}
