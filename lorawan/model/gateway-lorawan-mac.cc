...
GatewayLorawanMac::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::GatewayLorawanMac")
    ...
    .AddTraceSource ("GWReceivedPacket",
                     "Trace source that is fired when a packet arrives at the Gateway",
                     MakeTraceSourceAccessor (&GatewayLorawanMac::m_receivedPacket),
  ...
  if (macHdr.IsUplink ())
    {
      ...
      uint32_t gatewayId = m_device->GetObject<LoraNetDevice> ()->GetNode ()->GetId ();

      m_receivedPacket (packet, gatewayId);
    }
  else
    {
